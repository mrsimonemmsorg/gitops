module "argo" {
  ...
  redirect_uris = [
    "https://argo.azure.kubefirst.simonemms.com/oauth2/callback",
  ]
...
module "argocd" {
  ...
  oidc_provider_key_name = vault_identity_oidc_key.key.name
  redirect_uris = [
    "https://argocd.azure.kubefirst.simonemms.com/auth/callback",
  ]
  secret_mount_path = vault_mount.secret.path
}
...
module "console" {
  ...
  oidc_provider_key_name = vault_identity_oidc_key.key.name
  redirect_uris = [
    "https://kubefirst.azure.kubefirst.simonemms.com/api/auth/callback/vault",
  ]
  secret_mount_path = vault_mount.secret.path
}
